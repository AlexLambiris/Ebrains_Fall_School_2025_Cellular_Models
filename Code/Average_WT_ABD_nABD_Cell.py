...
class Average_ABD_nABD_WT_Cell_model(Cell):
    name = "Average_WT_Cell"
    ...
    def create_sections(self):
        ...
        self.ABD_tert = [h.Section(name=f'axoD_sec[{i}]') for i in range(4)]

        # Branches nABD
        self.nABD = [h.Section(name=f'nABD[{i}]') for i in range(3)]
        self.nABD_sec = [h.Section(name=f'nABD_sec[{i}]') for i in range(6)]
